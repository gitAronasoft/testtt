-- Add stripe_customer_id column to users table for customer management
ALTER TABLE users ADD COLUMN IF NOT EXISTS stripe_customer_id VARCHAR(255);
ALTER TABLE users ADD INDEX IF NOT EXISTS idx_stripe_customer_id (stripe_customer_id);

-- Create payment_disputes table for handling Stripe disputes
...
-- Create webhook_events table for logging and preventing duplicate processing
...
-- Update stripe_payments table to support webhook data
...
-- Ensure purchases table has payment_intent_id for webhook correlation
