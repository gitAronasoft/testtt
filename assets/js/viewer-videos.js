    ...
    fetch('/api.php?action=get_videos')
        ...
        .then(data => {
            if (data.success) {
                ...
                console.log('Loaded videos from database:', allVideos.length);
                ...
                if (allVideos.length > 0) {
                    showNotification(`Loaded ${allVideos.length} videos successfully`, 'success', 2000);
                }
            } else {
        ...
        .catch(error => {
            ...
            if (loading) loading.style.display = 'none';
            showNotification('Using demo videos (database connection failed)', 'warning', 3000);
        });
}
...
function renderVideos() {
    ...
    videosGrid.innerHTML = filteredVideos.map(video => {
        ...
        const isFree = parseFloat(video.price) === 0;

        return `
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="card video-card h-100 border-0 shadow-sm">
                    <div class="video-thumbnail position-relative bg-light d-flex align-items-center justify-content-center" style="height: 200px; font-size: 3rem;">
                        ${video.thumbnail || '🎥'}
                        <div class="position-absolute bottom-0 end-0 m-2 badge bg-dark">${video.duration || '10:00'}</div>
                        ${isFree ? 
                            '<div class="position-absolute top-0 end-0 m-2 badge bg-success">FREE</div>' :
                            isPurchased ? 
                                '<div class="position-absolute top-0 end-0 m-2 badge bg-info">✓ OWNED</div>' :
                                `<div class="position-absolute top-0 end-0 m-2 badge bg-primary">$${video.price}</div>`
                        }
                    </div>
                    <div class="card-body d-flex flex-column">
                        <h6 class="video-title fw-bold mb-2">${video.title}</h6>
                        <p class="video-description text-muted small mb-3 flex-grow-1">${truncateText(video.description || '', 80)}</p>
    ...
    if (videoDetails) {
        videoDetails.innerHTML = `
            <div class="d-flex align-items-center mb-3">
                <div class="video-thumbnail-small me-3" style="font-size: 2rem;">${video.thumbnail}</div>
                <div>
...
function processPayment() {
    if (!currentVideoForPurchase) {
        showNotification('No video selected for purchase', 'error');
        return;
    }
    ...
    setTimeout(() => {
        try {
            const purchase = {
                ...
                price: parseFloat(currentVideoForPurchase.price) || 0,
            ...
            showNotification(`Purchase successful! You can now watch "${purchase.videoTitle}".`, 'success');
            renderVideos();
...
function watchVideo(videoId) {
    ...
    if (!isFree && !isPurchased) {
        showNotification('Please purchase this video first to watch it', 'warning');
        return;
    }

    // Update view count locally (demo simulation)
    video.views = (video.views || 0) + 1;

    showNotification(`Now playing: "${video.title}"`, 'success');

    // In a real implementation, you would:
    // 1. Open video player modal/page
    // 2. Stream the actual video content
