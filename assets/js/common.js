...
function checkAuth() {
    ...
    if (window.location.href.includes('dashboard.html') && !isLoggedIn) {
        window.location.replace('login.html');
        return;
    }

    // Only redirect from login to dashboard if explicitly logged in
    if (isLoggedIn && currentPage === 'login.html' && localStorage.getItem('shouldRedirectToDashboard') === 'true') {
        localStorage.removeItem('shouldRedirectToDashboard');
        window.location.replace('dashboard.html');
        return;
    }

    // Check subscription for viewers
    if (isLoggedIn && currentPage === 'dashboard.html' && userRole === 'viewer' && !isSubscribed) {
        setTimeout(() => {
